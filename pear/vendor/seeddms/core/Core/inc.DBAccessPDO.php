...
class SeedDMS_Core_DatabaseAccess {
	...
	function __construct($driver, $hostname, $user, $passw, $database = false) { /* {{{ */
		...
		$this->_useviews = false; // turn off views, because they are much slower then temp. tables. They also break the transaction management, because dropping a view will commit the current transaction.
		$this->_debug = false;
	} /* }}} */
	...
	function connect() { /* {{{ */
		...
		try {
			...
			if (!$this->_conn)
				return false;
			/* Prevent PDO from throwing an exception because the code currently
			 * cannot handle it. PDO::ERRMODE_EXCEPTION became the default as of php 8.0.0
			 * PDO::ERRMODE_SILENT was the default before php 8.0.0
			 */
...
//					$this->_conn->setAttribute(PDO::ATTR_AUTOCOMMIT, FALSE);
	 ...
	 */
	function qstr(?string $text): string { /* {{{ */
		return is_null($text) ? 'NULL' : $this->_conn->quote($text);
	} /* }}} */
	...
	function getResultArray($queryStr, $retick=true) { /* {{{ */
		...
		if($this->_logfp) {
			fwrite($this->_logfp, microtime(true)."	".($this->_conn->inTransaction() ? '*' : ' ')." ".$queryStr."\n");
		}
...
//		$res->Close();
	...
	function startTransaction() { /* {{{ */
		...
		$this->_intransaction++;
		if($this->_logfp) {
			fwrite($this->_logfp, microtime(true)."	".($this->_conn->inTransaction() ? '*' : ' ')." START ".$this->_intransaction."\n");
		}
	} /* }}} */

	function rollbackTransaction() { /* {{{ */
		if($this->_logfp) {
			fwrite($this->_logfp, microtime(true)."	".($this->_conn->inTransaction() ? '*' : ' ')." ROLLBACK ".$this->_intransaction."\n");
		}
	...
	function commitTransaction() { /* {{{ */
		if($this->_logfp) {
			fwrite($this->_logfp, microtime(true)."	".($this->_conn->inTransaction() ? '*' : ' ')." COMMIT ".$this->_intransaction."\n");
		}
		if($this->_intransaction == 1) {
			$this->_conn->commit();
		}
...
//						"ORDER BY `maxLogID`";
					$dropStr = "DROP TABLE IF EXISTS `ttreviewid`";
				break;
				case 'pgsql':
...
//						"ORDER BY `maxLogID`";
				...
				default:
					$queryStr = "CREATE TEMPORARY TABLE IF NOT EXISTS `ttreviewid` (PRIMARY KEY (`reviewID`), INDEX (`maxLogID`)) ".
						...
						"GROUP BY `tblDocumentReviewLog`.`reviewID` "; //.
//						"ORDER BY `maxLogID`";
					$dropStr = "DROP TEMPORARY TABLE IF EXISTS `ttreviewid`";
			}
			if (!$this->_ttreviewid) {
...
//						"ORDER BY `maxLogID`";
					$dropStr = "DROP TABLE IF EXISTS `ttapproveid`";
					break;
				case 'pgsql':
...
//						"ORDER BY `maxLogID`";
				...
				default:
					$queryStr = "CREATE TEMPORARY TABLE IF NOT EXISTS `ttapproveid` (PRIMARY KEY (`approveID`), INDEX (`maxLogID`)) ".
						...
						"GROUP BY `tblDocumentApproveLog`.`approveID` "; //.
//						"ORDER BY `maxLogID`";
					$dropStr = "DROP TEMPORARY TABLE IF EXISTS `ttapproveid`";
			}
...
//						"ORDER BY `maxLogID`";
					$dropStr = "DROP TABLE IF EXISTS `ttstatid`";
					break;
				case 'pgsql':
...
//						"ORDER BY `maxLogID`";
				...
				default:
					$queryStr = "CREATE TEMPORARY TABLE IF NOT EXISTS `ttstatid` (PRIMARY KEY (`statusID`), INDEX (`maxLogID`)) ".
						...
						"GROUP BY `tblDocumentStatusLog`.`statusID` "; //.
//						"ORDER BY `maxLogID`";
					$dropStr = "DROP TEMPORARY TABLE IF EXISTS `ttstatid`";
			}
			if (!$this->_ttstatid) {
				if (!$this->getResult($queryStr))
					return false;
		...
		elseif (!strcasecmp($tableName, "ttcontentid")) {
			switch($this->_driver) {
				case 'sqlite':
					...
					$dropStr = "DROP TABLE IF EXISTS `ttcontentid`";
					break;
				case 'pgsql':
				...
				default:
					$queryStr = "CREATE TEMPORARY TABLE IF NOT EXISTS `ttcontentid` (PRIMARY KEY (`document`), INDEX (`maxVersion`)) ".
						...
						"GROUP BY `tblDocumentContent`.`document` ".
						"ORDER BY `tblDocumentContent`.`document`";
					$dropStr = "DROP TEMPORARY TABLE IF EXISTS `ttcontentid`";
			}
	...
	private function __dropTemporaryTable($tableName) { /* {{{ */
		$queryStr = '';
		if($this->_driver == 'sqlite' || $this->_driver == 'pgsql')
			$t = '';
		else
			$t = 'TEMPORARY';
		if (!strcasecmp($tableName, "ttreviewid")) {
			$queryStr = "DROP ".$t." TABLE IF EXISTS `ttreviewid`";
		} elseif (!strcasecmp($tableName, "ttapproveid")) {
			$queryStr = "DROP ".$t." TABLE IF EXISTS `ttapproveid`";
		} elseif (!strcasecmp($tableName, "ttstatid")) {
			$queryStr = "DROP ".$t." TABLE IF EXISTS `ttstatid`";
		} elseif (!strcasecmp($tableName, "ttcontentid")) {
			$queryStr = "DROP ".$t." TABLE IF EXISTS `ttcontentid`";
		}
		if($queryStr) {
			if (!$this->getResult($queryStr))
				return false;
	...
	private function __createView($tableName, $override=false) { /* {{{ */
		if (!strcasecmp($tableName, "ttreviewid")) {
			switch($this->_driver) {
				case 'sqlite':
					$queryStr = "CREATE VIEW IF NOT EXISTS `ttreviewid` AS ".
						"SELECT `tblDocumentReviewLog`.`reviewID` AS `reviewID`, ".
						"MAX(`tblDocumentReviewLog`.`reviewLogID`) AS `maxLogID` ".
						"FROM `tblDocumentReviewLog` ".
						"GROUP BY `tblDocumentReviewLog`.`reviewID` "; //.
...
//						return false;
		...
		elseif (!strcasecmp($tableName, "ttapproveid")) {
			switch($this->_driver) {
				case 'sqlite':
					$queryStr = "CREATE VIEW IF NOT EXISTS `ttapproveid` AS ".
						"SELECT `tblDocumentApproveLog`.`approveID` AS `approveID`, ".
						"MAX(`tblDocumentApproveLog`.`approveLogID`) AS `maxLogID` ".
						"FROM `tblDocumentApproveLog` ".
						"GROUP BY `tblDocumentApproveLog`.`approveID` "; //.
...
//						return false;
		...
		elseif (!strcasecmp($tableName, "ttstatid")) {
			switch($this->_driver) {
				case 'sqlite':
					$queryStr = "CREATE VIEW IF NOT EXISTS `ttstatid` AS ".
						"SELECT `tblDocumentStatusLog`.`statusID` AS `statusID`, ".
						"MAX(`tblDocumentStatusLog`.`statusLogID`) AS `maxLogID` ".
						"FROM `tblDocumentStatusLog` ".
						"GROUP BY `tblDocumentStatusLog`.`statusID` ";
...
//						return false;
		...
		elseif (!strcasecmp($tableName, "ttcontentid")) {
			switch($this->_driver) {
				case 'sqlite':
					$queryStr = "CREATE VIEW IF NOT EXISTS `ttcontentid` AS ".
						"SELECT `tblDocumentContent`.`document` AS `document`, ".
						"MAX(`tblDocumentContent`.`version`) AS `maxVersion` ".
						"FROM `tblDocumentContent` ".
						"GROUP BY `tblDocumentContent`.`document` ".
		...
		foreach($tables as $table) {
			...
			foreach($records as $record) {
				...
				foreach ($record as $column) {
					if (is_null($column)) $values .= 'NULL';
					elseif (is_numeric($column)) $values .= $column;
