...
class DocumentTest extends SeedDmsTest
{
    ...
    public function testGetAndSetAccess()
    {
         ...
         * is just a precaution to ensure the unlimited access rights is not
        ...
        /* Add unlimited access on the document for group */
         ...
         * to has unlimited rights. The specific user rights has higher priority.
         */
        $mode = $document->getAccessMode($user);
        $this->assertEquals(M_READWRITE, $mode);
        ...
        /* change unlimited access on the document for group to none */
        $ret = $document->changeAccess(M_NONE, $group->getId(), false);
        $this->assertEquals(M_ALL, $ret);
        $mode = $document->getAccessMode($user);
        $this->assertEquals(M_NONE, $mode);
